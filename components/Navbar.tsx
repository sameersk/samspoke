    ...
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-sm border-b border-slate-100">
      <nav className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
        <Link href="/" className="flex items-center gap-2 group">
          <span className="w-8 h-8 rounded-full bg-teal-600 flex items-center justify-center text-white font-bold text-sm group-hover:bg-teal-700 transition-colors">S</span>
          <span className="font-bold text-slate-900 text-lg tracking-tight">Sam&apos;s Spoke</span>
        </Link>
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link href="/about" className="text-sm font-medium text-slate-600 hover:text-teal-600 transition-colors">About</Link>
        </div>
        <div className="flex items-center gap-3">
          <Link href="#newsletter" className="btn-primary hidden md:inline-flex">Subscribe</Link>
          <button className="md:hidden p-2 rounded-md text-slate-600 hover:text-teal-600 hover:bg-slate-50 transition" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle menu">
            {menuOpen ? (
